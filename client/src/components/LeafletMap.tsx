...
type TileLayerConfig = { url: string; attribution: string; maxZoom: number };
...
const TILE_LAYERS: Record<MapType, TileLayerConfig> = {
  roadmap: {
...
export function resolveTileLayer(mapType: MapType | undefined): TileLayerConfig {
  return TILE_LAYERS[mapType ?? "roadmap"] ?? TILE_LAYERS.roadmap;
}
...
export function LeafletMap({ center, zoom, mapType = "roadmap", incidents, teams, onRetryGoogle }: LeafletMapProps) {
  ...
  useEffect(() => {
    ...
    if (!map) return;
    const layer = resolveTileLayer(mapType);
    ...
    <div className="absolute inset-0 overflow-hidden bg-slate-100">
      ...
      <div role="status" className="pointer-events-none absolute bottom-4 left-4 flex max-w-[calc(100%-2rem)] items-center gap-2 rounded-lg border border-emerald-200/90 bg-emerald-50/95 px-2.5 py-2 text-xs text-emerald-950 shadow-sm backdrop-blur">
        <span className="font-medium">
          OpenStreetMap ativo · {visibleIncidents} ocorrência(s) · {visibleTeams} equipe(s)
        </span>
        {onRetryGoogle && (
