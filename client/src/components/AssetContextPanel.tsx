...
export default function AssetContextPanel({ incidentReference, searchAssets, linkAsset, inventoryUrl, onLocateAsset }: AssetContextPanelProps) {
      ...
      <CardContent className="p-5">
        ...
        {!error && !loading && results.length > 0 && !selected && (
          <div className="mt-4 space-y-2">
            {results.map(asset => (
              <button key={asset.id} type="button" aria-label={`Selecionar ${asset.code}`} onClick={() => { setSelected(asset); setLinked(false); }} className="w-full rounded-lg border border-slate-200 p-3 text-left hover:bg-slate-50">
        ...
        {selected && (
          <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <Button type="button" size="sm" variant="outline" asChild><a href={`${inventoryBase}/assets/${encodeURIComponent(selected.id)}`} target="_blank" rel="noreferrer"><ExternalLink className="mr-2 h-3.5 w-3.5" />Abrir prontuário</a></Button>
            </div>
          </div>
        )}
      </CardContent>
