...
function IncidentDetailContent() {
  ...
  const recordCommunication = trpc.communications.recordIncidentEvent.useMutation({ onSuccess: () => { utils.incidents.timeline.invalidate({ incidentId }); utils.communications.history.invalidate({ incidentId }); if (canAudit) utils.incidents.audit.invalidate({ incidentId }); } });
