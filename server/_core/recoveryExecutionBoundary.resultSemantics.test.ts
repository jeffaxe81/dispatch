...
function harness(executor: RecoveryExecutorPort) {
...
describe("D-011B.4 executor result semantics", () => {
  it.each([
    ["simulated_success", "SIMULATED_FAILURE"],
    ["simulated_failure", "SIMULATED_SUCCESS"],
    ["simulated_timeout", "SIMULATED_CANCELLED"],
    ["simulated_cancelled", "SIMULATED_TIMEOUT"],
  ] as const)("fails closed for incompatible %s/%s pair", async (status, reasonCode) => {
    ...
    expect(result).toEqual({ status: "failed", reasonCode: "EXECUTOR_RESULT_INVALID" });
