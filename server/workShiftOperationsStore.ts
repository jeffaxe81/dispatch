...
export type SlaPolicy = { id:number; tenantId:number; anomalyType:WorkShiftAnomalyType|null; severity:WorkShiftAnomalySeverity|null; warningAfterMinutes:number|null; criticalAfterMinutes:number|null; escalationAfterMinutes:number|null; active:boolean };
...
const DEFAULT_SLA: Omit<EffectiveSlaPolicy,"tenantId"> = { id:0, anomalyType:null, severity:null, warningAfterMinutes:null, criticalAfterMinutes:null, escalationAfterMinutes:null, active:false, source:"default" };
...
export function createWorkShiftOperationsStore(persistence:WorkShiftOperationsPersistence){
  ...
  return {
    async upsertPendingFromAnomaly(anomaly:WorkShiftAnomalyCandidate):Promise<PendingRecord>{
      ...
      const slaDueAt=sla.criticalAfterMinutes!=null&&sla.criticalAfterMinutes>0?new Date(anomaly.detectedAt.getTime()+sla.criticalAfterMinutes*60_000):null;
