...
export type FormSubmissionSnapshot = { id: number; tenantId: number; formId: number; formVersionId: number; revision: number; status: "not_started" | "filling" | "submitted" | "corrected"; answers: FormAnswers };
...
export function createFormRepository(tenantId: number, adapter: FormRepositoryAdapter) {
  return {
    ...
    createBinding(input: { formId: number; formVersionId: number; contextType: FormContextType; contextId: string; createdByUserId: number }) { return adapter.createBinding({ tenantId, ...input }); },
    createSubmission(input: { formId: number; formVersionId: number; createdByUserId: number; answers: FormAnswers }) { return adapter.createSubmission({ tenantId, ...input }); },
    appendRevision(input: { submissionId: number; revision: number; answers: FormAnswers; reason: string; actorUserId: number }) { return adapter.appendRevision({ tenantId, ...input }); },
