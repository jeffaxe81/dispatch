...
export async function createFormsRequestContext(
  user: User | null,
  ports: FormsContextPorts,
  selectedTenantId?: number | null,
) {
  if (!user) throw new Error("Usuário autenticado é obrigatório para acessar formulários.");
  const tenantId = await ports.resolveTenantId({ userId: user.id, teamId: user.teamId, selectedTenantId });
  if (!Number.isInteger(tenantId) || tenantId <= 0) throw new Error("Tenant válido é obrigatório para acessar formulários.");
  const service = ports.createService(tenantId);
  return {
    tenantId,
    userId: user.id,
    service,
