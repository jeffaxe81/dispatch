...
const createScheduleInput = z.object({ code: z.string().trim().min(1).max(64), name: z.string().trim().min(1).max(160), organizationId: z.number().int().positive(), organizationalUnitId: z.number().int().positive().nullable(), scheduleType: z.enum(["fixed", "cyclic_12x36", "custom_cycle"]), timezone: z.string().trim().min(1).max(64), startTimeLocal: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/), weekdays: z.array(z.number().int().min(0).max(6)).max(7).nullable(), plannedDurationMinutes: z.number().int().positive(), breakPolicyMinutes: z.number().int().min(0).nullable(), cycleAnchorAt: z.date().nullable(), cycleWorkMinutes: z.number().int().positive().nullable(), cycleRestMinutes: z.number().int().positive().nullable(), effectiveFrom: z.date(), effectiveUntil: z.date().nullable() });
const assignmentInput = z.object({ scheduleId: z.number().int().positive(), userId: z.number().int().positive(), teamId: z.number().int().positive().nullable(), effectiveFrom: z.date(), effectiveUntil: z.date().nullable() });
const exceptionInput = z.object({ assignmentId: z.number().int().positive(), exceptionType: z.enum(["day_off", "replacement_shift", "leave", "extra_call", "holiday_override"]), startsAt: z.date(), endsAt: z.date(), reason: z.string().trim().max(500).nullable() });
...
export function createWorkShiftSchedulesRouter(deps: WorkShiftSchedulesRouterDependencies) {
  ...
  function assertRequestedOrganization(activeTenantId: number | null, requested?: number) {
    if (requested !== undefined && requested !== activeTenantId) throw new TRPCError({ code: "FORBIDDEN", message: "A escala pertence a outra empresa." });
  }

  return router({
    list: protectedProcedure.input(scheduleScopeInput).query(async ({ ctx, input }) => {
      const actor = await actorFor(ctx, "work_shift_schedules.view");
      assertRequestedOrganization(actor.organizationId, input.organizationId);
      return deps.listSchedules({ ...input, organizationId: actor.organizationId ?? undefined }, actor);
    }),
    create: protectedProcedure.input(createScheduleInput).mutation(async ({ ctx, input }) => {
      const actor = await actorFor(ctx, "work_shift_schedules.manage");
      assertRequestedOrganization(actor.organizationId, input.organizationId);
    ...
    coverage: protectedProcedure.input(coverageInput).query(async ({ ctx, input }) => {
      const actor = await actorFor(ctx, "work_shift_schedules.view");
      assertRequestedOrganization(actor.organizationId, input.organizationId);
      return deps.coverage({ ...input, organizationId: actor.organizationId ?? undefined }, actor);
    }),
  });
}
