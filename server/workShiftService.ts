...
async function resolvePlanningForStart(planningResolver: WorkShiftPlanningResolver | undefined, userId: number, now: Date) {
...
async function hydratePlanningSnapshot(current: OpenWorkShiftSnapshot) {
  const adapterAlreadySelectedSnapshot = current.scheduleAssignmentId !== undefined || current.scheduledStartAt !== undefined || current.scheduledEndAt !== undefined;
...
function resolveStartPlanningSnapshot(plan: ResolvedUserWorkShiftPlan | null, now: Date) {
  ...
  return { scheduleAssignmentId: plan?.assignmentId ?? null, scheduledStartAt, scheduledEndAt, lateStartSeconds: scheduledStartAt ? elapsedSeconds(scheduledStartAt, now) : 0, earlyEndSeconds: 0, overtimeSeconds: 0 };
}

function enrichEndPatch(current: OpenWorkShiftSnapshot, patch: WorkShiftSessionPatch, now: Date): WorkShiftSessionPatch {
  ...
  const plannedWorkSeconds = scheduledStartAt && scheduledEndAt ? elapsedSeconds(scheduledStartAt, scheduledEndAt) : 0;
  return { ...patch, earlyEndSeconds: scheduledEndAt ? elapsedSeconds(now, scheduledEndAt) : 0, overtimeSeconds: plannedWorkSeconds > 0 ? Math.max(0, workedSeconds - plannedWorkSeconds) : 0 };
}

export function snapshotOpenSession(value: OpenWorkShiftSnapshot | null): WorkShiftEventSnapshot | null {
  if (!value) return null;
  return { id:value.id, startedAt:value.startedAt.toISOString(), pausedAt:value.pausedAt?.toISOString() ?? null, endedAt:value.endedAt?.toISOString() ?? null, status:value.status, pausedSeconds:value.pausedSeconds, scheduleAssignmentId:value.scheduleAssignmentId ?? null, scheduledStartAt:value.scheduledStartAt?.toISOString() ?? null, scheduledEndAt:value.scheduledEndAt?.toISOString() ?? null, lateStartSeconds:value.lateStartSeconds ?? 0, earlyEndSeconds:value.earlyEndSeconds ?? 0, overtimeSeconds:value.overtimeSeconds ?? 0 };
}

function snapshotCreatedSession(sessionId: number, input: WorkShiftCreateSession): WorkShiftEventSnapshot {
  return { id:sessionId, userId:input.userId, teamId:input.teamId, source:input.source, startedAt:input.startedAt.toISOString(), pausedAt:null, endedAt:null, status:input.status, pausedSeconds:input.pausedSeconds, workedSeconds:input.workedSeconds, scheduleAssignmentId:input.scheduleAssignmentId, scheduledStartAt:input.scheduledStartAt?.toISOString() ?? null, scheduledEndAt:input.scheduledEndAt?.toISOString() ?? null, lateStartSeconds:input.lateStartSeconds, earlyEndSeconds:input.earlyEndSeconds, overtimeSeconds:input.overtimeSeconds };
}

function snapshotUpdatedSession(current: OpenWorkShiftSnapshot, patch: WorkShiftSessionPatch): WorkShiftEventSnapshot {
...
async function publishPersistedOperationalEvent(input: { tenantId?: number; userId: number; teamId: number | null; sessionId: number; eventType: WorkShiftEventType; occurredAt: Date; snapshot: WorkShiftEventSnapshot }) {
  if (input.tenantId === undefined) return;
  await publishWorkShiftOperationalEvent({ tenantId:input.tenantId, userId:input.userId, teamId:input.teamId, sessionId:input.sessionId, eventType:input.eventType, occurredAt:input.occurredAt, snapshot:input.snapshot });
}

export async function executeOwnWorkShiftAction(
  store: WorkShiftStore,
  input: { tenantId?: number; userId: number; teamId: number | null; action: WorkShiftAction; source?: WorkShiftSource; now?: Date },
  ...
  if (transition.mode === "create") {
    ...
    const createInput: WorkShiftCreateSession = { userId:input.userId, teamId:input.teamId, source, ...transition.session, ...resolveStartPlanningSnapshot(plan, now) };
    const created = await store.createSession(createInput);
    const afterData = snapshotCreatedSession(created.id, createInput);
  ...
  const effectiveCurrent = transition.sessionPatch.status === "ended" ? await hydratePlanningSnapshot(current) : current;
  ...
  const afterData = snapshotUpdatedSession(effectiveCurrent, sessionPatch);
