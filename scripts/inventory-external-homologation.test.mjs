...
import { runInventoryExternalHomologation } from "./inventory-external-homologation.mjs";
...
describe("inventory external homologation harness", () => {
  it("valida fluxo feliz, tenant isolation, replay e fail-closed", async () => {
    ...
    await withServer((req, res) => {
      ...
      if (req.url === "/api/v1/assets/asset-1") {
        res.end(JSON.stringify({ id: "asset-1", code: "PST-001", status: "ativo" }));
        return;
      }
      if (req.url === "/api/v1/assets/asset-1/location") {
      ...
      if (req.url === "/api/v1/assets/asset-1" && req.headers["x-tenant-id"] === "tenant-b") {
        res.statusCode = 404;
  ...
  it("reporta falha controlada quando o Motor fica indisponível sem travar o harness", async () => {
    ...
    expect(report.checks.some(check => check.name === "motor-unavailable" && check.status === "PASS")).toBe(true);
  });
});
