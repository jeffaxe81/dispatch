...
const profile = resolve(`.tmp-neo-auth-smoke-${process.pid}`);

const username = process.env.NEO_AUTH_USERNAME?.trim();
const password = process.env.NEO_AUTH_PASSWORD;

if (!username || !password) {
  console.log("neo_authenticated_login_smoke={\"status\":\"skipped-no-credentials\"}");
  process.exit(0);
}
...
try {
  ...
  if (navigation.errorText) {
    console.log("neo_authenticated_login_smoke={\"status\":\"navigation-error\"}");
    process.exitCode = 2;
  } else {
    ...
    if (probe.status !== "login-form-found") {
      console.log(`neo_authenticated_login_smoke=${JSON.stringify({ status: probe.status })}`);
      if (probe.status !== "authenticated") process.exitCode = 3;
    } else {
      ...
      let status = "login-rejected-or-pending";
      for (let attempt = 0; attempt < 60; attempt += 1) {
        ...
        try { sameOrigin = new URL(state.url).origin === NEO_ORIGIN; } catch {}
        if (!state.hasPassword && sameOrigin) {
      ...
      console.log(`neo_authenticated_login_smoke=${JSON.stringify({ status })}`);
      if (status !== "authenticated" && status !== "interactive-auth-required") process.exitCode = 4;
    }
  }
} finally {
  socket?.close();
