...
CREATE TABLE `operational_presence` (
	...
	`team_id` int,
	`work_session_id` int,
	`operational_presence_status` enum('available','busy','paused','offline','out_of_shift') NOT NULL DEFAULT 'offline',
	...
	CONSTRAINT `operational_presence_user_team_unique` UNIQUE(`user_id`,`team_id`)
);
--> statement-breakpoint
CREATE TABLE `route_track_points` (
	...
	CONSTRAINT `route_track_points_track_location_unique` UNIQUE(`route_track_id`,`team_location_id`)
);
--> statement-breakpoint
...
--> statement-breakpoint
ALTER TABLE `embedded_integrations` ADD CONSTRAINT `embedded_integrations_conn_fk` FOREIGN KEY (`integration_connection_id`) REFERENCES `integration_connections`(`id`) ON DELETE set null ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `operational_presence` ADD CONSTRAINT `operational_presence_user_id_users_id_fk` FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `shift_templates` ADD CONSTRAINT `shift_templates_organization_id_organizations_id_fk` FOREIGN KEY (`organization_id`) REFERENCES `organizations`(`id`) ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX `shift_schedules_status_start_idx` ON `shift_schedules` (`shift_schedule_status`,`scheduled_start_at`);--> statement-breakpoint
CREATE INDEX `shift_templates_active_idx` ON `shift_templates` (`active`);--> statement-breakpoint
