...
export const formEventTypeEnum = mysqlEnum("form_event_type", [
  "form.published",
  "submission.started",
...
export const formBindings = mysqlTable("form_bindings", {
  ...
  contextType: formContextTypeEnum.notNull(),
  contextId: varchar("context_id", { length: 180 }).notNull(),
  ...
  index("form_bindings_tenant_context_idx").on(table.tenantId, table.contextType, table.contextId),
  index("form_bindings_tenant_form_idx").on(table.tenantId, table.formId, table.formVersionId),
]);
...
export const formDomainEvents = mysqlTable("form_domain_events", {
  ...
  tenantId: int("tenant_id").notNull().references(() => organizations.id),
  eventType: formEventTypeEnum.notNull(),
  aggregateType: formEventAggregateTypeEnum.notNull(),
  aggregateId: varchar("aggregate_id", { length: 180 }).notNull(),
  actorUserId: int("actor_user_id").notNull().references(() => users.id),
  payload: json("payload").$type<Record<string, unknown>>().notNull(),
  occurredAt: timestamp("occurred_at").notNull(),
  deliveryStatus: formEventDeliveryStatusEnum.notNull().default("pending"),
  ...
  uniqueIndex("form_domain_events_event_id_unique").on(table.eventId),
  index("form_domain_events_delivery_idx").on(table.tenantId, table.deliveryStatus, table.nextAttemptAt),
  index("form_domain_events_aggregate_idx").on(table.tenantId, table.aggregateType, table.aggregateId),
]);
