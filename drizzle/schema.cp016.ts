...
export const shiftScheduleStatusEnum = mysqlEnum("shift_schedule_status", ["scheduled", "active", "completed", "cancelled"]);
export const workSessionStatusEnum = mysqlEnum("work_session_status", ["open", "paused", "closed", "adjusted"]);
export const workSessionSourceEnum = mysqlEnum("work_session_source", ["manual", "schedule", "integration", "admin_adjustment"]);
export const workSessionEventTypeEnum = mysqlEnum("work_session_event_type", ["start", "pause", "resume", "end", "adjustment"]);
export const operationalPresenceStatusEnum = mysqlEnum("operational_presence_status", ["available", "busy", "paused", "offline", "out_of_shift"]);
export const routeTrackStatusEnum = mysqlEnum("route_track_status", ["active", "completed", "cancelled"]);
export const embeddedIntegrationDisplayModeEnum = mysqlEnum("embedded_integration_display_mode", ["iframe", "external"]);
...
export const shiftTemplates = mysqlTable(
  ...
  table => [
    ...
    index("shift_templates_active_idx").on(table.active),
  ],
);
...
export const operationalPresence = mysqlTable(
  ...
  table => [
    uniqueIndex("operational_presence_user_team_unique").on(table.userId, table.teamId),
    index("operational_presence_team_dispatch_status_idx").on(table.teamId, table.availableForDispatch, table.status),
  ],
);
...
export const routeTrackPoints = mysqlTable(
  ...
  table => [
    ...
    uniqueIndex("route_track_points_track_location_unique").on(table.routeTrackId, table.teamLocationId),
  ],
);
...
export const embeddedIntegrations = mysqlTable(
  ...
  table => [
    foreignKey({
      ...
      name: "embedded_integrations_conn_fk",
    }).onDelete("set null"),
